...
class CDTrainer():
    ...
    def __init__(self, args, dataloaders):
        ...
        # Note: Moved to _backward_G() as the choice wasn't working when it was here.
        """
        if args.loss == 'ce':
            #self._pxl_loss = cross_entropy
            self._pxl_loss1 = cross_entropy
            self._pxl_loss2 = None
            ...
            self._pxl_loss1 = losses.multi_cross_entropy
            self._pxl_loss2 = None
        elif args.loss == 'ce_dice':
            #self._pxl_loss = losses.ce_dice
            self._pxl_loss1 = losses.ce_dice
            self._pxl_loss2 = None
        elif args.loss == 'dice+focal':
            self._pxl_loss1 = losses.diceloss_harvey
    ...
    def _collect_running_batch_states(self):
        ...
        if np.mod(self.epoch_id, 20) == 1:
            ...
            vis_pred = utils.make_numpy_grid(self._visualize_pred(), pad_value=5, padding=5)
            ...
            axes[2].imshow(vis_pred[:, :, 0], cmap=cmap, vmin=0, vmax=5)
            axes[3].imshow(vis_gt[:, :, 0], cmap=cmap, vmin=0, vmax=5)
    ...
    def _backward_G(self):
        ...
        if self.args.loss == 'ce':
            ...
            self._pxl_loss1 = losses.focal_loss
            self._pxl_loss2 = None
        elif self.args.loss == 'ce_multi':
            self._pxl_loss1 = losses.multi_cross_entropy
            self._pxl_loss2 = None
        elif self.args.loss == 'ce_dice':
            self._pxl_loss1 = losses.ce_dice
            self._pxl_loss2 = None
        elif self.args.loss == 'dice+focal':
            self._pxl_loss1 = losses.diceloss_harvey
            self._pxl_loss2 = losses.focal_loss
        else:
            raise NotImplemented(self.args.loss)
    
        if gt.shape[0] != 1:
            if self._pxl_loss2 != None:
                self.G_loss = self._pxl_loss1(self.G_pred, gt) + self._pxl_loss2(self.G_pred, gt)
            else:
                self.G_loss = self._pxl_loss1(self.G_pred, gt)
        else:
