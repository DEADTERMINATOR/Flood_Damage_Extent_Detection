...
def cross_entropy(input, target, weight=None, reduction='mean',ignore_index=255):
    ...
    #return F.cross_entropy(input=input, target=target, weight=None,
    #                       ignore_index=ignore_index, reduction=reduction)
...
def diceloss(input, target, weight=None, mode='binary'):
    input = torch.argmax(input, dim=1).type(torch.float32)
    target = target.type(torch.float32)
...
def diceloss_harvey(input, target, weight=None):
    #Make the number of channels match the number of classes by duplicating the tensor across additional channels.
    input = torch.argmax(input, dim=1, keepdim=True).type(torch.float32).repeat_interleave(repeats=5, dim=1)

    diceloss = smp_losses.DiceLoss(mode='multiclass', from_logits=False)
    loss = diceloss(input, target)
    return loss